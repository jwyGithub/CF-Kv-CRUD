...
var KVController = class {
  ...
  async getItem(key, ...options) {
    try {
      ...
      const { metadata } = await this.kv.getWithMetadata(key, { cacheTtl: 60 });
  ...
  async getAll() {
    ...
    const result = [];
    for await (const key of keys) {
      const { value, metadata } = await this.kv.getWithMetadata(key.name, { cacheTtl: 60 });
...
var route_default = [
  {
    ...
    async excute(request, _env) {
      try {
        ...
        const req = new Request(HOME_PAGE, request);
        const indexHtml = await (await fetch(req)).text();
